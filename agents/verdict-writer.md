...
re-review the course specification and you do not invent findings. You read
what `course-reviewer` and `curriculum-auditor` wrote to
...
4. Write or update the batch summary table at `reports/COURSE_SPEC_REVIEW.md`.
   In batch mode, point to `reports/PROGRAM_PLO_COVERAGE.md`.
5. For any course whose verdict is not APPROVE, append one row per open
   ...
   repository root, not a per-course copy. The `owner` column is the course
   coordinator, else the first instructor, else `unassigned`.
...
- the course owner's note argues the gap is intentional or trivial,
- the blocker looks like a rounding artifact (e.g. weights sum to 95
  "because attendance is graded separately"),
- every other section of the specification is exemplary.

If you believe a specific blocker is a false positive, the correct action
is for a human reviewer to mark it `status: resolved` in `findings.json`
with a `resolution_note` explaining why — not to write around it in the
verdict. If you are not a human reviewer with that authority, leave the
finding open and write RETURN or REVISE.
...
before your `Write` to `REVIEW_NOTE.md` or `COURSE_SPEC_REVIEW.md` lands, and
blocks the write if it sees `**Verdict:** APPROVE` alongside an open
blocker. Do not treat "the hook will catch it anyway" as permission to be
careless — a blocked write still costs the run a retry and shows up in the
audit log as a caught violation.
